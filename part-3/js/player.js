...
myPlayer.init = function(targetContainer){
	...
	$(document).on('fullscreenchange mozfullscreenchange msfullscreenchange webkitfullscreenchange', $.proxy(this.fullscreenChange, this));
};
...
myPlayer.onPlaying = function(){
	if(!this.seeking){
		this.$container.removeClass(this.states).addClass('playing');
	}
};
myPlayer.onPause = function(){
	if(!this.seeking){
		this.$container.removeClass(this.states).addClass('paused');
	}
};
